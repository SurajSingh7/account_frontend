...
import CalculationBreakdown from './CalculationBreakdown' // ✅ Import separate component
...
const calculateBalanceWithBreakdown = (order, fromDate, toDate, splitFactor = 1, stateToShow = '') => {
...
const calculateBalance = (order, fromDate, toDate, splitFactor = 1) => {
  const breakdown = calculateBalanceWithBreakdown(order, fromDate, toDate, splitFactor);
...
const TableRow = React.memo(({
  ...
  splitFactor = 1,
  stateToShow,
  onViewBreakdown // ✅ NEW: Callback to view breakdown
}) => {
  const servicePeriod = getServicePeriod(order, toDateFormatted);
  const breakdown = calculateBalanceWithBreakdown(order, fromDateFormatted, toDateFormatted, splitFactor, stateToShow);
    ...
    <tr className="hover:bg-blue-50/40 transition-colors duration-150 border-b border-slate-100 last:border-0">
      ...
      <td className="px-4 py-4 text-right">
        <div className="flex items-center justify-end gap-2">
          ...
          <button
            onClick={() => onViewBreakdown(breakdown)} // ✅ NEW: Pass breakdown data
...
const BillingReportComp = () => {
  ...
  const [selectedBreakdown, setSelectedBreakdown] = useState(null); // ✅ NEW: State for breakdown view
  ...
  const totalBalance = useMemo(() => {
    return filteredOrders.reduce((sum, order) => {
      ...
      return sum + calculateBalance(order, fromDateFormatted, toDateFormatted, 1);
    }, 0);
  ...
  // ✅ NEW: If breakdown is selected, show full-page breakdown view
                    ...
                    return splitBilling ? (
                      <React.Fragment key={order.id}>
                        <TableRow
                          ...
                          onViewBreakdown={setSelectedBreakdown} // ✅ Pass callback
                        />
                        <TableRow
                          order={order}
                          hideLsiColumn={hideLsiColumn}
