...
// ─── SERVER-SIDE GST RECALCULATION ────────────────────────────────────────────
// Ensures all credit notes and misc sell items have correct GST fields.
// Called before any save/update to guarantee data integrity in DB.
...
const recalcCreditNotes = (creditNotes, isSelfGST) => {
  ...
  return creditNotes.map((cn) => {
    ...
    // Always recompute if totalWithGst is missing or zero while amount exists.
    // Also recompute if GST type fields look inconsistent.
    ...
    if (needsRecalc) {
      ...
      return {
        date: cn.date || '',
        periodStart: cn.periodStart || '',
        periodEnd: cn.periodEnd || '',
        amount: amt,
        cgst,
        sgst,
        igst,
    ...
    // Even if GST is already correct, ensure all schema fields are present
    return {
      date: cn.date || '',
      periodStart: cn.periodStart || '',
      periodEnd: cn.periodEnd || '',
      ...
      totalWithGst: Number(cn.totalWithGst) || 0,
      invoiceNumber: cn.invoiceNumber || '',
      notes: cn.notes || '',
    };
  });
...
const recalcMiscSell = (miscellaneousSell, isSelfGST) => {
  if (!miscellaneousSell || miscellaneousSell.length === 0) return [];
  return miscellaneousSell.map((item) => {
    ...
    if (needsRecalc) {
      ...
      const igst = isSelfGST ? 0 : Math.round(amt * 0.18 * 100) / 100;
      const totalWithGst = Math.round((amt + cgst + sgst + igst) * 100) / 100;
      return { date: item.date || '', amount: amt, cgst, sgst, igst, totalWithGst, notes: item.notes || '' };
    }
    ...
    return {
      ...
      totalWithGst: Number(item.totalWithGst) || amt,
      notes: item.notes || '',
    };
  });
};

// ── Sanitise a full billing object before writing to DB
  ...
  while (currentDate <= serviceEndDate && currentDate <= currentEndDate) {
    ...
    if (shouldSplit) {
      ...
      const billing1 = {
        ...
        endDate: endDateStr,
        billingDays,
        perDayRate: totalAmount * state1Percentage / 100 / daysInMonth,
        receivedDetails: [],
        creditNotes: [],
        miscellaneousSell: [],
        tdsProvision: [],
        ...
        sgst: sgst1,
        igst: igst1,
        totalWithGst: monthlyCharge1 + cgst1 + sgst1 + igst1,
        invoiceNumber: '',
      ...
      const billing2 = {
        ...
        endDate: endDateStr,
        billingDays,
        perDayRate: totalAmount * state2Percentage / 100 / daysInMonth,
        receivedDetails: [],
        creditNotes: [],
        miscellaneousSell: [],
        tdsProvision: [],
        ...
        sgst: sgst2,
        igst: igst2,
        totalWithGst: monthlyCharge2 + cgst2 + sgst2 + igst2,
        invoiceNumber: '',
      ...
      const billing = {
        ...
        endDate: endDateStr,
        billingDays,
        perDayRate: totalAmount / daysInMonth,
        receivedDetails: [],
        creditNotes: [],
        miscellaneousSell: [],
        tdsProvision: [],
        ...
        sgst,
        igst,
        totalWithGst: monthlyCharge + cgst + sgst + igst,
        invoiceNumber: '',
        isSelfGST,
...
// ── KEY FIX: sanitiseBillingForSave() is called before writing to DB.
//    This ensures credit notes and misc sell always have correct GST fields
//    saved in MongoDB, regardless of how the data was originally entered.
export async function PUT(request) {
  try {
    ...
    // ── Recalculate GST for credit notes and misc sell before saving
...
// PATCH - Backfill GST for ALL existing credit notes in DB that are missing it.
// Call this once: POST /api/billing/monthly?action=backfill
// After running, all historical records in DB will have correct GST fields.
export async function PATCH(request) {
  try {
    ...
    if (action !== 'backfill') {
      return NextResponse.json({ success: false, error: 'Unknown action. Use ?action=backfill' }, { status: 400 });
    }

    // Find all billings that have credit notes with missing totalWithGst
    const billings = await MonthlyBilling.find({
      'creditNotes.0': { $exists: true }, // has at least one credit note
    });
    ...
    for (const billing of billings) {
      ...
      const needsFix = fixed.some((cn, i) => {
        const orig = billing.creditNotes[i];
        return (Number(orig.totalWithGst) || 0) !== cn.totalWithGst;
      });
      ...
      if (needsFix) {
        ...
        bulkOps.push({
          updateOne: {
            filter: { _id: billing._id },
            update: { $set: { creditNotes: fixed } },
          },
        });
      }
    }
    ...
    if (bulkOps.length > 0) {
      await MonthlyBilling.bulkWrite(bulkOps);
    }

    return NextResponse.json({
      success: true,
      message: `Backfill complete. Fixed ${fixedCount} billing records.`,
      fixed: fixedCount,
