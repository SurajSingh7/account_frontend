...
export default function DistributedPaymentsPage() {
  ...
  const fetchRecords = useCallback(async () => {
    ...
    try {
      ...
      const res  = await fetch(`${API_ENDPOINTS.internal.purchase.distributed}?${params}`)
  ...
  const handleDelete = async (id) => {
    try {
      const res  = await fetch(`${API_ENDPOINTS.internal.purchase.distributed}?id=${id}`, { method: 'DELETE' })
      const json = await res.json()
