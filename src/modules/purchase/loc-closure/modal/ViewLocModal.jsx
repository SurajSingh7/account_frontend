...
const ViewLocModal = () => {
  ...
  const fetchOrder = async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`${API_BACKEND_URL}${API_ENDPOINTS.purchase.readyOrder.base}/${locId}`, {
        credentials: 'include',
      });
      if (!res.ok) {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <SectionCard icon={<Building2 className={SECTION_ICON_CLS} />} title="Basic Info">
            <div className="grid grid-cols-2 gap-x-6 gap-y-4">
              ...
              <Detail label="LOC Effective" value={formatDateDisplay(order.locDate)} />
              <Detail label="LOC Closing" value={formatDateDisplay(order.operationalDate)} />
            </div>
          </SectionCard>
