...
// ── Credit Notes total: always use totalWithGst (basic+GST).
//    If totalWithGst=0 but amount exists (old DB records without GST), derive it.
...
// Build a CN invoice number pre-filled with the billing month
      ...
      <div className="max-w-7xl mx-auto p-6 lg:p-8 space-y-6">
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-200 p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            {/* Monthly Billing Basic — editable */}
            <div>
...
const MonthlyBillGeneratorComp = () => {
  ...
  const fetchOrderDetails = async () => {
    try {
      ...
      const res = await fetch(`${API_ENDPOINTS.internal.purchase.orders}?orderId=${orderId}`);
  ...
  const fetchBillings = async () => {
    try {
      const res = await fetch(`${API_ENDPOINTS.internal.purchase.monthly}?orderId=${orderId}`);
      const result = await res.json();
      if (result.success) {
  ...
  const handleAutoGenerate = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(API_ENDPOINTS.internal.purchase.monthly, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ orderId, mode: 'auto', autoInvoice: true }) });
      const result = await res.json();
  ...
  const handleDelete = async (billingId) => {
    ...
    try {
      const res = await fetch(`${API_ENDPOINTS.internal.purchase.monthly}?billingId=${billingId}`, { method: 'DELETE' });
      const result = await res.json();
  ...
  const handleDeleteAll = async () => {
    ...
    try {
      const res = await fetch(`${API_ENDPOINTS.internal.purchase.monthly}?orderId=${orderId}`, { method: 'DELETE' });
      const result = await res.json();
  ...
  const handleSaveEdit = async (editFormData) => {
    try {
      const res = await fetch(API_ENDPOINTS.internal.purchase.monthly, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(editFormData) });
      const result = await res.json();
  ...
  if (!orderId) return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/20 to-slate-50 flex items-center justify-center p-6">
      <div className="bg-white rounded-2xl shadow-xl border border-red-200 p-10 max-w-lg w-full text-center">
        ...
        <Link href={ROUTES.purchase.ordersLink} className="inline-flex items-center gap-2 px-8 py-4 bg-gradient-to-r from-blue-600 to-blue-700 text-white rounded-xl font-semibold shadow-lg"><ArrowLeft className="w-5 h-5" />Go to Orders</Link>
      </div>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/20 to-slate-50">
      <div className="max-w-[1900px] mx-auto p-6 lg:p-8 space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            <Link href={ROUTES.purchase.outstandingReport} className="p-2.5 hover:bg-white rounded-xl border border-gray-200 transition-all hover:shadow-md group">
              <ArrowLeft className="w-5 h-5 text-gray-600 group-hover:text-blue-600 transition-colors" />
            </Link>
            <div>
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-200 p-6 hover:shadow-xl transition-shadow">
          <div className="flex items-center justify-between mb-6">
            ...
            <span className={`px-4 py-2 rounded-full text-sm font-bold ${orderDetails.status === 'LOC' ? 'bg-gradient-to-r from-green-100 to-emerald-100 text-green-700 border border-green-300' : 'bg-gradient-to-r from-red-100 to-rose-100 text-red-700 border border-red-300'}`}>{orderDetails.status}</span>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-6">
            ...
            <div className="space-y-1"><p className="text-xs text-gray-500 font-semibold uppercase tracking-wider">LOC Date</p><p className="text-base font-bold text-gray-900 flex items-center gap-1"><CalendarIcon className="w-4 h-4 text-blue-600" />{formatDateToDisplayLocal(orderDetails.locDate)}</p></div>
