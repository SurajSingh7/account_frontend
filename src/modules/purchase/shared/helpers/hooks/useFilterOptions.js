...
export const useStates = () => {
  ...
  useEffect(() => {
    const fetch_ = async () => {
      setLoading(true);
      try {
        const res  = await fetch(`${API_BACKEND_URL}${API_ENDPOINTS.purchase.config.indianStates}`);
...
export const useEntities = () => {
  ...
  useEffect(() => {
    const fetch_ = async () => {
      setLoading(true);
      try {
        const res  = await fetch(`${API_BACKEND_URL}${API_ENDPOINTS.purchase.entity.all}`, { credentials: 'include' });
        const json = await res.json();
...
export const useBsos = () => {
  ...
  useEffect(() => {
    const fetch_ = async () => {
      setLoading(true);
      try {
        const res  = await fetch(`${API_BACKEND_URL}${API_ENDPOINTS.purchase.bso.parentAll}`,{credentials:'include'});
        const json = await res.json();
...
export const useCompanies = (search = '') => {
  ...
  useEffect(() => {
    ...
    const fetch_ = async () => {
      setLoading(true);
      try {
        const url  = `${API_BACKEND_URL}${API_ENDPOINTS.purchase.companyGroup.all}${search ? `?search=${encodeURIComponent(search)}` : ''}`;
