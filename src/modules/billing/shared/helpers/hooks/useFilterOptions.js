...
export const useStates = () => {
  ...
  useEffect(() => {
    const fetch_ = async () => {
      setLoading(true);
      try {
        const res  = await fetch(`${API_BACKEND_URL}/config/indian-states/`);
...
export const useBsos = () => {
  ...
  useEffect(() => {
    const fetch_ = async () => {
      setLoading(true);
      try {
        const res  = await fetch(`${API_BACKEND_URL}/bso/parent/all/`,{credentials:'include'});
        const json = await res.json();
...
export const useCompanies = (search = '') => {
  ...
  useEffect(() => {
    ...
    const fetch_ = async () => {
      setLoading(true);
      try {
        const url  = `${API_BACKEND_URL}/company/group/all${search ? `?search=${encodeURIComponent(search)}` : ''}`;
