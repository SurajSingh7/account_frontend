...
async function putLedgerEntry(id, payload) {
  const res = await fetch(`${API_BACKEND_URL}/billing/sale/ledger/modify/${id}`, {
...
async function moveProvisionToConfirm({ monthlyBillingId, notes }) {
  const res = await fetch(`${API_BACKEND_URL}/billing/sale/ledger/move-provision-to-confirm/`, {
    method: 'POST', headers: apiHeaders(), credentials: 'include',
...
async function fetchCreditNoteAmount({ startDate, endDate, monthlyBillingId }) {
  const res = await fetch(`${API_BACKEND_URL}/billing/sale/ledger/calculate-credit-note-amount`, {
    method: 'POST', headers: apiHeaders(), credentials: 'include',
