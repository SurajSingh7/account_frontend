...
export const enrichOutstandingLedger = (rows = [], apiTotals = {}) => {
  ...
  const enrichedRows = rows.map((row) => {
    ...
    return {
      ...row, // ← all existing fields untouched
      runningOutstanding: Math.round(runningOutstanding),
      outstandingAfterAdjustment: Math.round(outstandingAfterAdjustment),
    }
  })
  ...
  return {
    ...
    totals: {
      ...apiTotals, // ← all existing totals untouched
      runningOutstanding: Math.round(
        lastRow?.runningOutstanding || 0
      ),
      outstandingAfterAdjustment: Math.round(
        lastRow?.outstandingAfterAdjustment || 0
      ),
    },
  }
}