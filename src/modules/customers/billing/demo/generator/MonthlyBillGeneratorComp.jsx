...
const MonthlyBillGeneratorComp = () => {
  ...
  const fetchOrderDetails = async () => {
    try {
      ...
      const res = await fetch(`${API_ENDPOINTS.internal.billing.orders}?orderId=${orderId}`);
  ...
  const fetchBillings = async () => {
    try {
      const res = await fetch(`${API_ENDPOINTS.internal.billing.monthly}?orderId=${orderId}`);
      const result = await res.json();
      if (result.success) {
  ...
  const handleAutoGenerate = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(API_ENDPOINTS.internal.billing.monthly, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ orderId, mode: 'auto', autoInvoice: true }) });
      const result = await res.json();
  ...
  const handleDelete = async (billingId) => {
    ...
    try {
      const res = await fetch(`${API_ENDPOINTS.internal.billing.monthly}?billingId=${billingId}`, { method: 'DELETE' });
      const result = await res.json();
  ...
  const handleDeleteAll = async () => {
    ...
    try {
      const res = await fetch(`${API_ENDPOINTS.internal.billing.monthly}?orderId=${orderId}`, { method: 'DELETE' });
      const result = await res.json();
  ...
  const handleSaveEdit = async (editFormData) => {
    try {
      const res = await fetch(API_ENDPOINTS.internal.billing.monthly, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(editFormData) });
      const result = await res.json();
  ...
  if (!orderId) return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/20 to-slate-50 flex items-center justify-center p-6">
      <div className="bg-white rounded-2xl shadow-xl border border-red-200 p-10 max-w-lg w-full text-center">
        ...
        <Link href={ROUTES.customers.billing.ordersLink} className="inline-flex items-center gap-2 px-8 py-4 bg-gradient-to-r from-blue-600 to-blue-700 text-white rounded-xl font-semibold shadow-lg"><ArrowLeft className="w-5 h-5" />Go to Orders</Link>
      </div>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/20 to-slate-50">
      <div className="max-w-[1900px] mx-auto p-6 lg:p-8 space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            <Link href={ROUTES.customers.billing.outstandingReport} className="p-2.5 hover:bg-white rounded-xl border border-gray-200 transition-all hover:shadow-md group">
              <ArrowLeft className="w-5 h-5 text-gray-600 group-hover:text-blue-600 transition-colors" />
            </Link>
            <div>
